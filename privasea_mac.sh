...
if [[ "$1" != "child-run" ]]; then
  ...
  echo "=== Sudo 권한이 필요할 수 있습니다. 비밀번호를 입력해 주세요. ==="
  sudo -v   # 여기서 비밀번호 입력

  echo "현재 screen 세션이 아닙니다. 새로운 screen 세션($SCREEN_NAME)에서 스크립트를 실행합니>다."
...
echo ""
echo "Docker 버전 확인:"
docker --version || echo "[WARN] Docker 데몬이 실행 중이 아닐 수 있습니다. Docker Desktop을 열
어주세요."
echo ""
...
if ! docker images | grep -q "privasea/acceleration-node-beta"; then
  ...
  # ARM Mac에서 x86 이미지만 존재할 경우 --platform linux/amd64 붙일 수 있음
...
################################################
PRIVASEA_DIR="$HOME/privasea/config"
if [ ! -d "$PRIVASEA_DIR" ]; then
  echo "$PRIVASEA_DIR 디렉토리가 없어서 생성합니다."
  mkdir -p "$PRIVASEA_DIR"
  chmod 755 "$PRIVASEA_DIR"
else
  echo "$PRIVASEA_DIR 디렉토리가 이미 존재합니다."
fi
...
################################################
set +e
if [ ! -f "$PRIVASEA_DIR/wallet_keystore" ]; then
  echo "wallet_keystore 파일이 없으므로 새 지갑을 생성합니다."
  echo "이 과정에서 비밀번호를 직접 입력해야 합니다."
  docker run -it -v "$PRIVASEA_DIR:/app/config" "$IMAGE_NAME" ./node-calc new_keystore
  ...
  echo "이미 $PRIVASEA_DIR/wallet_keystore 파일이 존재합니다. 건너뜁니다."
fi
...
if [ -n "$RUNNING_CONTAINER" ]; then
  ...
  echo "KEYSTORE_PASSWORD를 입력해주세요:"
  read -s KEYSTORE_PASSWORD
  echo
  
  docker run -d \
    -v "$PRIVASEA_DIR:/app/config" \
